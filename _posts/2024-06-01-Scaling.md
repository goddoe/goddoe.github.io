...
비슷한 AI 관련 문제를 해결하고 있는 분들에게 도움이 되고자 내 경험과 인사이트를 공유하고 싶다. 모든 경험과 노하우를 한 번에 쓰기보다는 조금씩 나눠서 공유할 계획이다.
...
문제 해결을 위한 이러한 스케일링은 AI/ML 외의 다른 분야에서도 관찰된다. 반도체에서는 매년 나노 공정을 통해 회로 선폭을 더 얇게 만들어 회로 밀도를 높이는 방식으로 회로를 스케일링한다. GPU는 CUDA 코어 수를 늘리는 방식으로 스케일링된다.
